...
//                break;
    ...
    public function displayLogin(){
        ...
        // Load CSS
        $PAGE->requires->css('/local/parentportal/css/styles.css');
        $PAGE->requires->css('/local/parentportal/css/custom-theme/jquery-ui-1.10.3.custom.css');

        // Load javascript modules
        $PAGE->requires->js_call_amd('local_parentportal/scripts', 'init', array(
          array(
            'dateFormat' => $this->getDateFormat()
          )
        ));

        // Old scripts file with functions
        $PAGE->requires->js('/local/parentportal/scripts.js');

        // Display moodle header with all the relevant scripts
        echo $OUTPUT->header();
    ...
    private function loadPage($page, $TPL)
    {
        ...
        switch($page)
        {
            ...
            case 'coursereports':
                ...
                try {
                    $OBJ = \ELBP\Plugins\Plugin::instaniate("CourseReports", null, false);
                    if ($OBJ){
                      if ($this->student)
                      {
            ...
            case 'attendance':
                ...
                try {
                    $ATT = \ELBP\Plugins\Plugin::instaniate("Attendance", null, false);
                    if ($ATT){
                      if ($this->student)
                      {
            ...
            case 'comments':
                ...
                try {
                    $Comments = \ELBP\Plugins\Plugin::instaniate("Comments", null, false);
                    if ($Comments){
                      if ($this->student)
                      {
            ...
            case 'main':
                ...
                if ($this->student)
                {
                    ...
                    // STudent Profile
                    $OBJ = \ELBP\Plugins\Plugin::instaniate("StudentProfile", null, false);
                    if ($OBJ){
                      if ($this->student)
                      {
                          $OBJ->loadStudent($this->student->id);
                    ...
                    // Course Reports
                    $CR = \ELBP\Plugins\Plugin::instaniate("CourseReports", null, false);
                    if ($CR){
                    ...
                    // GT
                    $GT = \ELBP\Plugins\Plugin::instaniate("elbp_gradetracker", null, false);
                    if ($GT)
                    {
            ...
            $output .= "<table>";
                ...
                if ($this->students)
                {
                    foreach($this->students as $student)
                    {
                        ...
                        {
                            $output .= "<tr><td colspan='2'><a href='{$link}' class='{$class}'>".$OUTPUT->user_picture($student, array("size" => 35, "link" => false)) . " " . \fullname($student)." <small>({$student->$field})</small></a></td><td><a title='{$this->string['deleterequest']}' href='#' student='{$student->id}' class='pp_delete_request'><img src='pix/delete.png' /></a></td></tr>";
                        }
                        elseif ($student->requestStatus == PP_STATUS_PENDING)
                        {
                            $output .= "<tr title='{$this->string['pendingresponse']}'><td>".$OUTPUT->user_picture($student, array("size" => 35, "link" => false)) . " " . \fullname($student)." <small>({$student->$field})</small></td><td><img src='pix/pending.png' /></td><td><a href='#' class='pp_delete_request' student='{$student->id}' title='{$this->string['cancelrequest']}'><img src='pix/delete.png' /></a></td></tr>";
                        }
